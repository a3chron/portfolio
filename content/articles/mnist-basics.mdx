...
This Data is then separated in data used for training the NN (~ 75%), and data used for evaluating the NN (~ 25%).

> [!NOTE] One time through all training data = "epoch"
...
> (a standard program is usually quicker to write, takes no time and money for training, is not as complex as a NN, and you have no room for errors)

> This would by the way be an extreme example for **overfitting**, i.e. the NN getting used to the data it is trained with, 
...
>
> In this picture, we can see three datasets.
> Let's say, the red points are images of dogs, while the green one are cats.
> (for the light mode ppl cyan and pink).  
> These points are our training data.  
> The line is where our NN is separating the images.
>
> In the left image, we can see the result of a NN, 
> that was probably not trained enough (or has not enough layers etc.).  
> => **Underfitting**
>
> In the middle one, there is an **overfitted** NN,
...
> but it is much too specialized to fit any other data.
>
> The image on the right side shows a balanced NN.
...
> i.e. it will hopefully perform wonderful on new data.
...
If you'd be sure, that *absolutely* all wolves will have snow in the background, while no dog will *ever* be seen together with snow,
...
> and so the AI did not know what to do when suddenly the grass disappeared at the bidge.
...
but you could also be responsible for much worse (self-driving cars crashing into each other, etc.).
...
That problem is one specific kind of biases. A NN with a bias basically means that the content of your data is represented in the responds the NN gives, 
e.g. the NN copies certain beliefs that are represented in the data. 
...
they are already separated into training and evaluation data, there is enough data (60.000 images) and so on... The perfect dataset.
...
[^1]: Stole this idea from [mikhail berkov's](https://github.com/uhasker/) book on [NextJS](https://uhasker.github.io/getting-things-done-in-next-js/).