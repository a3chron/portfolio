...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase
    .rpc("increment_vote", { p_article_id: slug, p_vote_type: "like" })
    .single();

  if (error) {
    console.error(error);
    return NextResponse.json({ msg: "failed to update likes" }, { status: 400 });
  }

  const counts = (data ?? {}) as { likes?: number; dislikes?: number };
  return NextResponse.json({ msg: "updated likes", ...counts }, { status: 201 });
}
